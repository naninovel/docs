...
Background music (BGM) and sound effects (SFX) are covered in this article; for the voices see [voicing guide](/zh/guide/voicing.md).
...
Audio playback behavior can be configured using `Naninovel -> Configuration -> Audio` context menu; for available options see [configuration guide](/zh/guide/configuration.md#audio). 
...
In C# scripts, currently used audio mixer can be retrieved via `IAudioManager` [engine service](/zh/guide/engine-services.md).
