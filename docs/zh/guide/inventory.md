...
`ItemExist()` and `ItemCount()` custom [expression functions](/zh/guide/script-expressions.md#expression-functions) to check wither an items exist in inventory and number of existing items are also available for convenience.
