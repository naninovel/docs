...
--- | ---
IBackgroundManager | Manages [background](/zh/guide/backgrounds.md) actors.
ICharacterManager | Manages [character](/zh/guide/characters.md) actors.
IChoiceHandlerManager | Manages [choice handler](/zh/guide/choices.md) actors.
ITextPrinterManager | Manages [text printer](/zh/guide/text-printers.md) actors.
IAudioManager | Manages the audio: [SFX](/zh/guide/audio.md#sound-effects), [BGM](/zh/guide/audio.md#background-music) and [voicing](/zh/guide/voicing.md).
IInputManager | Manages the user [input processing](/zh/guide/input-processing.md).
ILocalizationManager| Manages the [localization](/zh/guide/localization.md) activities.
ITextManager | Handles [managed text](/zh/guide/managed-text.md) feature.
IMoviePlayer | Handles [movie](/zh/guide/movies.md) playing.
IScriptManager | Manages [naninovel script](/zh/guide/naninovel-scripts.md) resources.
IScriptPlayer | Handles [naninovel scripts](/zh/guide/naninovel-scripts.md) execution.
...
IStateManager | Handles `IEngineService`-related persistent data de-/serialization; provides API to [save and load](/zh/guide/save-load-system.md) game state.
IUIManager | Manages `IManagedUI` objects and handles [UI customization](/zh/guide/user-interface.md#ui-customization) feature.
ICustomVariableManager | Provides access and allows modifying [custom variables](/zh/guide/custom-variables.md). 
ISpawnManager | Manages objects spawned with [@spawn] commands.
IUnlockableManager | Manages [unlockable items](/zh/guide/unlockable-items.md) (CG and movie gallery items, tips, etc).
...
In case your custom service has a persistent state, which you wish to de-/serialize with other engine services, implement `IStatefulService<TState>` interface, where `TState` is either `GameStateMap`, `GlobalStateMap` or `SettingsStateMap` depending if you want to store the state with the game sessions-specific, global state or settings. It's allowed to implement all three interfaces for a single service if required. For more information on different types of engine state see [state management guide](/zh/guide/state-management.md).
