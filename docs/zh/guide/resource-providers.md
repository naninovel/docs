...
| 资源类型    | 地址                                                             |
|---------|----------------------------------------------------------------|
| 场景脚本    | `Naninovel/Scripts/{SCRIPT_PATH}`                              |
| 精灵角色外观  | `Naninovel/Characters/{CHARACTER_ID}/{APPEARANCE_PATH}`        |
| 通用角色    | `Naninovel/Characters/{CHARACTER_ID}`                          |
| 音频      | `Naninovel/Audio/{AUDIO_PATH}`                                 |
| 托管文本文档  | `Naninovel/Text/{DOCUMENT_PATH}`                               |
...
- 不要在资源上下文的参数中使用 [表达式](/zh/guide/expressions)，例如 actor ID、外观、音频路径等。表达式在命令执行之前被评估，这使得无法在构建时解析最终路径。Naninovel 会在构建期间检测到此类情况时警告您。
