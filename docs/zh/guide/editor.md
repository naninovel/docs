...
嵌入模式使用操作系统的原生 WebView，首次启动时需要 Internet 连接来下载编辑器二进制文件。二进制文件会被缓存，因此同一项目中的后续运行不再需要网络连接。

::: tip
WebView 组件由 OS 维护，但在少数情况下，它可能被禁用或更新失败，导致故事编辑器无法正常工作。要在 Windows 上验证 WebView 版本，请在 PowerShell 中运行以下命令：
...
每个剧本编辑器选项卡代表一个 `.nani` 脚本，文本行显示为行。您可以使用选项卡底部的 `+` 按钮、上下文菜单（右键单击现有内容）或 `Enter` 热键（可在 [设置](/zh/guide/editor#键位映射-keymap) 中配置）添加行。

您可以通过拖放重新排序行，通过上下文菜单或使用 `Backspace` 和 `Ctrl+D` 键删除或复制行。

更改会立即应用，如果编辑的脚本当前正在播放，则会反映在游戏视图中。要保留更改，您需要 [保存](/zh/guide/editor#脏保存-dirty-save) 修改后的文件。
