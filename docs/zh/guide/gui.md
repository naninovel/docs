...
可以通过相机配置中的 `Aspect Ratios` 选项限制支持的纵横比。

![](https://i.gyazo.com/838cccc525d9d77f2d2951de7884ad72.png)

指定一个或多个纵横比时，Naninovel 会将游戏设置 UI 中可用的分辨率筛选为匹配的分辨率，并在由 Naninovel 管理的相机上强制使用最接近的已配置纵横比。该纵横比通过更改相机视口矩形来强制应用，并在需要时添加上下黑边或左右黑边。

纵横比以宽度和高度对的形式指定。例如，宽屏显示器使用 `16:9`，经典全屏布局使用 `4:3`。当当前屏幕纵横比未与任何已配置的比例完全匹配时，Naninovel 会选择最接近的已配置比例。

::: info NOTE
Unity 不会清除相机矩形之外的屏幕，因此这些像素可能会保留先前渲染的内容。为避免这种情况，请在 Naninovel 相机后方清除屏幕。最简单的设置是添加一个先于 Naninovel 渲染的相机：将 `Clear Flags` 设置为 `Solid Color`，将 `Background` 设置为所需的黑边颜色，将 `Culling Mask` 设置为 `Nothing`，并将 `Depth` 设置为低于 Naninovel 相机。
:::
