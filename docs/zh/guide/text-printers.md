...
在某些情况下，可能需要在生成的本地化文档中包含 [脚本表达式](/zh/guide/script-expressions) 或在语言（语言环境）更改时强制重新评估表达式。
...
— 每次将文本分配给打印机时（包括语言更改时），都会重新评估 `random(t_text1, t_text2)` 表达式，从而使 `t_text1` 和 `t_text2` [脚本文本变量](/zh/guide/managed-text#脚本文本) 与当前活动语言环境同步。表达式也将包含在本地化文档中，允许为每种特定语言更改它。
