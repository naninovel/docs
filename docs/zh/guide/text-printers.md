...
在某些情况下，可能需要在生成的本地化文档中包含 [剧本表达式](/zh/guide/expressions) 或在语言（区域设置）更改时强制重新评估表达式。
...
如果您需要更灵活的选择逻辑，请创建一个带有 `select` 别名的自定义 [表达式函数](/zh/guide/expressions#添加自定义函数)，并使其接受 `params string[]` 并返回 `string`。每当编译选择器标签时，它都会使用您的函数来评估结果。下面是自定义选择函数的示例，其中第一个选项指定选择类型，如果第一个选项为空，则回退到随机选择：
