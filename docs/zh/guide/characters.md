...
分配后，actor 将使用相机渲染内容，而不是自定义渲染过程，这将解除所有固有的限制，例如缺乏模板支持。缺点是，为了确保内容仅渲染到 actor 纹理（并且不会“泄漏”到主相机），您必须专门为渲染此类 actor 保留相机 [层](https://docs.unity3d.com/Manual/Layers.html)。
...
::: warning
Live2D SDK for Unity R5 的变更使我们无法继续维护与 Naninovel 的正确集成。如果您使用的是此 Live2D 版本，我们的集成将无法工作。请考虑改用 [通用](/zh/guide/characters#通用角色)、[分层](/zh/guide/characters#相机渲染)（相机模式）或自定义角色实现。
:::
