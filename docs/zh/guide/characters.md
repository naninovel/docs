...
::: info NOTE
在默认（非摄像机）渲染模式下，仅支持 `Sprite Renderer` 的 "simple" 绘制模式；当选择其他模式时，它们将像设置为 simple 一样进行渲染。
:::
...
请注意，层对象在运行时不会由 Unity 摄像机直接渲染；相反，它们在每次组合（外观）更改时渲染一次到临时渲染纹理，然后将其提供给 Naninovel 摄像机可见的自定义网格。此设置是防止半透明过度绘制问题和支持过渡动画效果所必需的。
...
如果您的角色预制件包含非平凡的渲染器，如粒子系统、轨迹、精灵遮罩或自定义/第三方渲染器，您仍然可以通过将摄像机分配给 `Layered Behaviour` 组件的 `Render Camera` 字段（摄像机必须在角色预制件内）来使用分层实现。

分配后，actor 将使用摄像机渲染内容，而不是自定义渲染过程，这将解除所有固有的限制，例如缺乏模板支持。缺点是，为了确保内容仅渲染到 actor 纹理（并且不会“泄漏”到主摄像机），您必须专门为渲染此类 actor 保留摄像机 [层](https://docs.unity3d.com/Manual/Layers.html)。

Unity 中共有 32 个层可用，其中 8 个保留给引擎内部使用。您可以随意使用剩余的层（默认情况下未使用）。要允许 Naninovel 使用层在摄像机模式下渲染分层 actor，请给层名称以 `Naninovel` 开头；例如 `Naninovel 1`、`Naninovel 2` 等。

![](https://i.gyazo.com/dfbb4306553c85a4683fffb0fef03de3.png)

要添加的总层数取决于在任何给定时间生成的唯一摄像机渲染分层 actor 的最大数量，无论可见与否。当分层 actor 被渲染时，它会从池中持有一个层。当 actor 被隐藏时，它会释放该层，允许其他 actor 重用它。但是，actor 在外观更改时也会被渲染，这发生在它们最初添加到场景时（即使它们被隐藏）或加载或回滚游戏时，因此 **层池大小必须适应一次生成的摄像机渲染 actor 的总数**，而不仅仅是一次可见的此类 actor 的数量。要在隐藏后销毁（取消生成）actor，请使用 [@remove] 命令。

添加层后，在 Naninovel 的摄像机配置中的 `Custom Camera Prefab` 分配一个摄像机预制件，该预制件的剔除掩码禁用了 `Naninovel ...` 层。这是防止“原始”分层 actor 泄漏到主 Naninovel 摄像机所必需的。请注意，分配的摄像机应该是专用的摄像机预制件，而不是您在分层 actor 预制件中使用的摄像机。

![](https://i.gyazo.com/e2f713e4e212718f50e028cdf546aaba.png)

在摄像机渲染模式下，分层 actor 预制件的游戏对象如果附加了 `Layered Actor Layer` 组件，则被视为层，其他对象被视为组。附加组件后，使用 `On Layer Held` 和 `On Layer Released` Unity 事件配置持有和释放摄像机层时应发生的情况。通常，您会将持有的层分配给宿主游戏对象并启用关联的渲染器，并在释放时禁用渲染器（以确保其他摄像机不会拾取该对象）。

![](https://i.gyazo.com/4dbfe57dbf6b7365e1e7db78f707f412.png)

层活动状态在摄像机模式下也有不同的反映：使用游戏对象的活动状态代替渲染器组件的启用状态。要设置 actor 的默认外观，请启用/禁用游戏对象。
...
确保 `Render Canvas` 组件附加到分层 actor 预制件根目录，因为摄像机模式需要它。它的工作原理类似于普通渲染模式，限制渲染纹理大小，因此请保持紧凑。

如果渲染 actor 需要额外的层（例如，专门用于 Unity 的 Lights 2D 的层），请通过分层行为组件上的 `Camera Mask` 属性添加它们。Naninovel 在渲染 actor 时将保留摄像机剔除掩码中的指定层。
...
::: warning
Live2D SDK for Unity R5 的变更使我们无法继续维护与 Naninovel 的正确集成。如果您使用的是此 Live2D 版本，我们的集成将无法工作。请考虑改用 [通用](/zh/guide/characters#通用角色)、[分层](/zh/guide/characters#摄像机渲染)（摄像机模式）或自定义角色实现。
:::
