...
| **boolean** | 逻辑值，可为 `true` 或 `false`。多数情况下可以使用 [布尔标志（Boolean Flags）](/guide/naninovel-scripts#boolean-flags) 代替输入 `true` 或 `false`，例如：`@camera ortho! !wait` 等价于 `@camera ortho:true wait:false`。 |
...
你可以在普通文本行的开头添加一个作者 ID，并使用冒号加空格（`: `）分隔，以将显示的文本与对应的 [角色](/guide/characters) 关联：
...
所有指令（包括 [内置指令](/api/) 和 [自定义指令](/guide/custom-commands)）都可以通过方括号（`[ ]`）语法内联到普通文本行中。
...
唯一需要使用完整布尔形式的情况，是当你希望通过 [脚本表达式](/guide/script-expressions) 动态计算布尔值时，例如：
...
可以使用 [@if] 和 [@else] 指令定义多行的条件代码块（可[嵌套](/guide/naninovel-scripts#nesting)使用）：
...
::: info
有关条件表达式及可用运算符的更多信息，请参阅 [脚本表达式](/guide/script-expressions) 指南。
:::
...
若脚本顶部包含 [注释行](/guide/naninovel-scripts#comment-lines)，对应的图形节点将显示这些注释作为摘要（Synopsis）。若不希望显示摘要，可在脚本配置菜单中取消勾选 “Show Synopsis” 选项。
...
若希望手动触发当前正在播放的 Naninovel 脚本的热重载（例如在 Unity 外部编辑脚本或在构建版本中运行时），可使用 `reload` [控制台指令](/guide/development-console)。
...
有关如何安装和使用此扩展的详细说明，请参阅 [IDE 扩展指南](/guide/ide-extension)。

## 脚本调试

在处理大型 Naninovel 脚本时，每次都从头开始播放来测试脚本的特定部分会非常麻烦。

你可以使用 [开发者控制台](/guide/development-console)，即时将当前播放的脚本“回退”到任意一行：
...
当前播放的脚本路径、行号及指令（或内联指令）索引会显示在窗口标题栏中。当启用 [自动语音](/guide/voicing#auto-voicing) 功能时，对应语音文件的名称也会显示在窗口中。  
...
Fountain 中的 [Action](https://fountain.io/syntax#section-action) 与 [Dialogue](https://fountain.io/syntax#section-dialogue) 段落会被转换为 [普通文本行](/guide/naninovel-scripts.html#generic-text-lines)，而其他语法结构则会被转换为 [注释行](/guide/naninovel-scripts.html#comment-lines)。
