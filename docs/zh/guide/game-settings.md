...
::: info 注意
要指定哪些文本元素会受到字体和文字大小更改的影响，请使用 UI 预制体（`CustomUI` 组件）的 `Font Change Configuration` 属性。默认情况下，仅文字输出窗会受影响。有关修改和创建自定义 UI 的更多信息，请参阅 [自定义指南](/zh/guide/gui#ui-customization)。
:::
