...
::: note
This configuration reference is valid for [Naninovel v1.13](https://github.com/Elringus/NaninovelWeb/releases).
:::
...
Type Namespaces | Object Ref | When looking for various types (eg, actor implementations, serialization handlers, managed text, etc) the engine will only scan the exported domain types under the specified namespaces for performance. In case you're keeping your custom implementations under a specific namespace, add it here.
