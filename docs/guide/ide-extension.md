...
::: tip
In case the metadata is not syncing, make sure `Enable Bridging` is enabled in the engine configuration and ensure `Generated Data Root` value shown at the top of the engine configuration menu equals the data root reported by the IDE extension.
:::
