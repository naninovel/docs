...
Total number of layers to add depends on the max amount of unique camera-rendered layered actors in a single scenario script. When layered actor is rendered, it holds a layer from the pool. When the actor hidden, it releases the layer, allowing it to be re-used by other actors. However, the actors are also rendered on appearance change, which happen when they're initially added to scene (even if they're initially hidden) or when game is loaded or rolled-back, so **layer pool size have to accommodate total number of camera-rendered actors in a scenario script**, and not just amount of actors visible at the same time.
