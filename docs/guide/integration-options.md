...
::: example
Check out [example project on GitHub](https://github.com/Elringus/NaninovelIntegrationExample), where Naninovel is used as both a drop-in dialogue for a 3D game and a standalone novel mode, switchable from the same 3D mode. Be aware, that Naninovel package is not distributed with the project, hence compilation errors will be produced after opening it for the first time; import Naninovel from the Asset Store to resolve the issues.
:::
