...
debug | Переключает окно [отладки сценариев Naninovel](/ru/guide/naninovel-scripts#отладка-сценариев).
var | Переключает окно[редактора пользовательских переменных](/ru/guide/custom-variables#отладка-переменных).
purge | Сбрасывает кэш загруженных ресурсов, когда используется провайдер [Google Drive](/ru/guide/resource-providers#google-drive).
...
reload | Исполняет [горячую перезагрузку](/ru/guide/naninovel-scripts#горячая-перезагрузка) проигрываемого в данный момент сценарая Naninovel. Работает только в редакторе Unity.
