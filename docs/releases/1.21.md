...
10. In case using `@animate` or `@delay` commands, replace them with [@async].
...
- Added `nowait!` flag to the [@<] command allowing to disable waiting for the text reveal to finish before proceeding with the playback.
...
- Changed the default alias of the parameter for disabling wait input (CTC) in the [@<] command from `skip!` to `noi!` (the old alias still works for backward compatibility).
