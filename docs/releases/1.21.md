...
7. In case using custom commands, update the `Execute` signature to accept `ExecutionContext` instead of `AsyncToken` and return `UnityEngine.Awaitable` instead of `Naninovel.UniTask`.
...
12. In case using engine's async APIs, migrate to the new `UnityEngine.Awaitable` from the old `Naninovel.UniTask` APIs; find related helpers under `Naninovel.Async` class.
