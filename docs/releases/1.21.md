...
8. In case using custom commands, update the `Execute` signature to accept `ExecutionContext` instead of `AsyncToken`.
9. In case using script playback APIs of `IScriptPlayer` service, use `IScriptPlayer.MainTrack` to get `IScriptTrack`, which now hosts most of the playback-related APIs. In custom commands, use `ExecutionContext.Track` to get the track associated with the command execution.
10. In case using `@delay` commands, replace them with [@async] followed by [@wait].
...
- Added proper support for the asynchronous scenario script execution via the updated `IScriptPlayer` APIs and the new `IScriptTrack` instances, which can be accessed from the player service.
- Transient scripts, such as played via `PlayScript` component and `IScriptPlayer.PlayTransient` method no longer have any limitations compared to actual script assets.
