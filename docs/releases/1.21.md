...
6. Choice buttons are now always loaded via the `Choice Button Loader` specified in the choice handler configuration menu, and should be stored in the `Resources/Naninovel/ChoiceButtons` folder. Alternatively, you can configure a different resource provider in the loader settings, for example to use Addressables.
...
- Changed the default alias of the inlined "wait input" (CTC) command from `[i]` to `[-]` and the default way of disabling waiting input at the end of generic lines from `[< skip!]` to `[>]`. The old way still works for backward compatibility.
