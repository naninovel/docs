...
| Task | ChangeAppearanceAsync(`String` appearance, `Single` duration, `TransitionType` transitionType, `Vector4` transitionParams) |  | 
...
| Type | Name | Summary | 
| --- | --- | --- | 
| Task | ChangeAppearanceAsync(`String` appearance, `Single` duration, `TransitionType` transitionType, `Vector4` transitionParams) | Changes background appearance over specified time with specified transition effect. | 
...
| Int32 | GetHashCode() |  | 
| Boolean | Sample() | Returns true when changed state from inactive to active since the last sample.  Returns false when changed state from active to inactive since the last sample.  Returns null when activation state hadn't changed since the last sample. | 
...
| Task | TransitionToAsync(`Texture` texture, `Single` duration, `Boolean` smoothStep = False, `TransitionType` transitionType = null, `Vector4` transitionParams = null) |  | 
...
| void | DestroyService() |  | 
| Task | InitializeServiceAsync() |  | 
| Task | LoadDefaultEngineStateAsync(`Func&lt;Task&gt;[]` additionalTasks) | Loads a default engine state.  Will reset the services and unload unused assets before load and mask the process with the loading UI (when available). | 
