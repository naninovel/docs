import { Context, CapturedAsset, DownloadedAsset, AssetType, ensureDir, wait } from "../common";
...
export async function download(assets: CapturedAsset[], ctx: Context): Promise<DownloadedAsset[]> {
    ...
    ctx.fetches.push(...fetching.values());
...
async function downloadAsset(asset: CapturedAsset): Promise<DownloadedAsset> {
    ...
    function fetchWithTimeout(uri: string, filepath: string): Promise<void> {
        ...
        try {
            return fetchAndWriteTo(uri, filepath, abort.signal);
        } finally {
