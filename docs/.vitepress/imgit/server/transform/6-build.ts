...
async function buildAsset(asset: EncodedAsset): Promise<BuiltAsset> {
    let html: string;
    if (asset.type === AssetType.Image) html = await cfg.builders.image(asset);
    else if (asset.type === AssetType.Animation) html = await cfg.builders.animation(asset);
    else if (asset.type === AssetType.Video) html = await cfg.builders.video(asset);
    else html = await cfg.builders.youtube(asset);
    return { ...asset, html };
}
...
export async function buildVideo(asset: EncodedAsset): Promise<string> {
    ...
    const cls = `class="${cfg.builders.style.className.video}"`;
    ...
    return `
<div class="${cfg.builders.style.className.container}">
...
export async function buildYouTube(asset: EncodedAsset): Promise<string> {
    const cls = `class="${cfg.builders.style.className.youtube}"`;
...
async function buildPicture(asset: EncodedAsset): Promise<string> {
    ...
    const cls = asset.type === AssetType.Image
        ? cfg.builders.style.className.image
        : cfg.builders.style.className.animation;
...
function serve(fullPath: string, asset: EncodedAsset): string | Promise<string> {
    ...
    if (cfg.servers === "auto") return `${cfg.host}/${relativePath}`;
    return cfg.servers(relativePath, asset);
}
...
async function buildPoster(asset: EncodedAsset, size: string): Promise<string> {
    ...
    const cls = cfg.builders.style.className.poster;
