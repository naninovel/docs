...
デフォルトでは、行やコマンドの種類を区別しやすくするために、行識別子の代わりに半透明のアイコンが表示されます。この動作は `Decoration Style` と `Decoration Opacity` の設定で調整できます。コマンドグループごとの色は、`semanticTokenColorCustomizations` 設定で個別にカスタマイズできます。
...
::: tip
式クエリパラメーターにも、同様のパラメーターコンテキスト属性のほとんどを適用して、IDE拡張機能での自動補完と診断を有効にすることができます。[クエリガイド](/ja/guide/expressions#パラメーターコンテキスト) の例を参照してください。
:::
