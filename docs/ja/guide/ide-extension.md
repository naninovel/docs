...
::: tip
式クエリパラメータにも、同様のパラメータコンテキスト属性のほとんどを適用して、IDE拡張機能での自動補完と診断を有効にすることができます。[クエリガイド](/ja/guide/expressions#パラメータコンテキスト) の例を参照してください。
:::

## 列挙型式

`EnumContext` IDE属性を使用する場合、列挙型の代わりに、コマンドパラメータ値やその他の変数（現在検査中のスクリプトなど）に基づいて列挙型名を生成するためにIDEによって評価される式を指定できます。
