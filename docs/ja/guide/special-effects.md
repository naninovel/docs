...
名前 | タイプ | デフォルト | 説明
--- | --- | --- | ---
Focus Object Name | String | null | フォーカスを設定するゲームオブジェクトの名前（任意）。 設定すると、フォーカスは常にゲームオブジェクトに留まり、 `Focus Distance` パラメーターは無視されます。
...
Duration | Decimal | 1 | 補間時間（パラメーターが目標値に到達する速さ）。
...
; カメラから10ユニット離れたところにフォーカスポイントを設定し、
; 焦点距離を0.95にし、3秒かけて適用
