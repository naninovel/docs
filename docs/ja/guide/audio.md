...
すべてのオーディオコマンドは `pos` パラメータをサポートしています。このパラメータを指定すると、空間モード（「3Dオーディオ」）が有効になり、関連付けられたオーディオソースはオーディオリスナーとの相対位置に反応するよう設定され、ワールド空間で指定された位置に配置されます。
...
他のパラメータと同じように、位置をアニメーションすることもできます。
