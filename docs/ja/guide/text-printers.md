...
::: note
表示イベントは一時的なトラック上でコマンドを実行し、メイントラックには影響を与えられないため、それらを待機することはできません（`wait!` フラグは無視されます）。また、"入力待ち"（CTC）を発生させることもできません。そのような場合は、代わりに[インラインコマンド](/ja/guide/scenario-scripting#コマンドのインライン化)を使用してください。
:::
