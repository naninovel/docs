...
レイヤー実装は [ユニバーサル](/ja/guide/backgrounds#ユニバーサル背景) をベースにしており、`Layer Actor Controller` コンポーネントも備えています。このコンポーネントを使用すると、複数のスプライト（レイヤー）から背景を作成し、実行時にシナリオスクリプトを介して個別に切り替えることができます。
