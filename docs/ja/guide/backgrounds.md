...
[サポートするアスペクト比を強制](/ja/guide/gui#アスペクト比の強制)することもできますが、よりアクセシブルな代替案は、ゲームを任意のアスペクト比に適応できるようにすることです。組み込みGUIはすでに柔軟で、任意の画面サイズに適応します。また、アクターの配置はシーン境界に対する相対位置であり、その境界は画面に合わせられます。背景の一致モードを使用すると、背景アクターが画面をどのように埋めるかを設定できます。
