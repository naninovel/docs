...
背景は、従来のVNゲームフローによりよく対応するために、キャラクターとは少し異なる方法で処理されます。ほとんどの場合、シーンには1つの背景アクターしかなく、常に異なる外観に遷移します。スクリプトで同じアクターIDを繰り返す手間を省くために、`MainBackground` アクターが影響を受けるものであると仮定して、背景の外観とトランジションタイプ（オプション）のみをプライマリパラメーターとして提供することが可能です。そうでない場合は、`id` パラメーターを介して背景アクターのIDを明示的に指定できます。
...
メイン背景アクターレコードは、背景リソースマネージャーでデフォルトで作成され、名前を変更したり削除したりすることはできません。ただし、メイン背景のパラメーター（実装、ピボット、PPUなど）は自由に変更できます。
...
背景や背景の外観が多く、エディターメニューですべてを割り当てるのが不便な場合は、アクターレコードアセット（`Create -> Naninovel -> Actor Record -> Background`）を使用してください。これらはマルチ編集をサポートしており、フォルダーでレコードを整理できます。例については、以下のビデオを確認してください。
...
[サポートするアスペクト比を強制](/ja/guide/gui#アスペクト比の強制)することもできますが、よりアクセシブルな代替案は、ゲームを任意のアスペクト比に適応できるようにすることです。組み込みGUIはすでに柔軟で、任意の画面サイズに適応します。また、アクターの配置はシーン境界に対する相対位置であり、その境界は画面に合わせられます。背景の一致モードを使用すると、背景アクターが画面をどのように埋めるかを設定できます。
...
ポーズ名は、[@back] コマンドで外観として使用して、コマンドパラメーターを介して個別に指定する代わりに、ポーズで指定されたすべての選択されたパラメーターを一度に適用できます。
...
Naninovelは背景プレースホルダーを自動的に生成しますが、背景エディターの `Placeholder Appearances` リストを使用して、特定の外観の見た目を定義できます。
...
オープンソースの [SpriteDicing](https://github.com/elringus/sprite-dicing) パッケージで構築された `DicedSpriteBackground` 実装により、関連するテクスチャにほとんど類似したデータが含まれている場合に、背景スプライトのテクスチャ領域を再利用することで、ビルドサイズとテクスチャメモリを大幅に削減できます。
...
WebGLでは、Unityのビデオプレイヤーはストリーミングモードでのみ動作するため、WebGLプレイヤーをビルドすると、すべてのビデオリソースが `Assets/StreamingAssets/Backgrounds` フォルダーにコピーされます。**StreamingAssets** フォルダーはビルド出力ディレクトリにも表示されます。ビルドを公開するときはそれを保持し、Webサーバーがこのフォルダーからのデータの読み取りを許可していることを確認してください。

コピーされたビデオファイルは、Unityによってトランスコードされません（オプションが有効になっている場合でも）。そのため、ソースファイルは最初からWebブラウザーでサポートされている形式である必要があります。または、ビルド後にゲームディレクトリ内のクリップファイルを置き換えることができます。以下は、WebGLデモで使用されている背景ビデオクリップの詳細なメタデータです。
...
背景を単純なスプライト以上のものにしたい場合、まず検討すべき選択肢がユニバーサル実装です。Unityで利用可能な標準レンダラー（メッシュ、パーティクル、スキニングされたスプライト、タイルマップなど）をすべてサポートします。さらに、ユニバーサルアクターのコンテンツはライトとボリュームの影響を受けます。

`Create -> Naninovel -> Background -> Universal` アセットコンテキストメニューを使用して、テンプレートから新しいユニバーサル背景プレハブを作成し、ダブルクリックしてプレハブ編集モードに入ります。ルートオブジェクトには `Universal Background Behaviour` コンポーネントがあります。これはNaninovelとプレハブコンテンツの間のアダプターです。

このプレハブはUnityの他のプレハブと同じように構築できます。Naninovelはプレハブルート以下にある互換性のあるすべてのレンダラーを取得し、実行時に背景用のレンダーテクスチャに合成します。コンポーネントの `On Appearance Changed` イベントを使用すると、外観の変更に対するコールバックを設定できます。たとえば、Unityの [Animator](https://docs.unity3d.com/Manual/class-Animator.html) システムを使用して背景のアニメーションを制御できます。
...
レイヤー実装は [ユニバーサル](/ja/guide/backgrounds#ユニバーサル背景) をベースにしており、`Layer Actor Controller` コンポーネントも備えています。このコンポーネントを使用すると、複数のスプライト（レイヤー）から背景を作成し、実行時にシナリオスクリプトを介して個別に切り替えることができます。
...
[@back] コマンドのプライマリパラメーターは外観とトランジションタイプを期待している（[@char] コマンドのようなIDと外観ではない）ことを忘れないでください。したがって、レイヤー合成式は次の方法で指定してください。
...
汎用背景は、最も柔軟な背景アクター実装です。これは、ルートオブジェクトに `Generic Background Behaviour` コンポーネントがアタッチされたプレハブに基づいています。外観の変更およびその他すべての背景パラメーターは [Unityイベント](https://docs.unity3d.com/Manual/UnityEvents.html) としてルーティングされ、基になるオブジェクトの動作を好きなように実装できます。
...
シーン背景設定には、デフォルトで `Assets/Scenes` に設定された `Scene Root Path` オプションがあります。これは、アクターのシーンアセットが配置されると予想されるディレクトリです。変更することも（たとえば、アクターごとに個別のフォルダーを指定するなど）、そのままにしておくこともできます。
