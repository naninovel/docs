...
レイヤー実装は [ユニバーサル](/ja/guide/characters#ユニバーサルキャラクター) をベースにしており、`Layer Actor Controller` コンポーネントも備えています。このコンポーネントを使用すると、複数のレイヤーからキャラクターを作成し、実行時にシナリオスクリプトを介して個別またはグループで切り替えることができます。
