...
すべてのカスタム変数は、ゲームと一緒に自動的に保存されます。デフォルトでは、変数はゲームセッションにローカルです。つまり、プレイ中に変数を代入したあとで、プレイヤーが新しいゲームを開始したり、その変数が代入されていない別のセーブスロットを読み込んだりすると、その値は失��れます。この動作は、ほとんどの種類の変数にとって便利です。変数をゲームセッションから "切り離したい" 場合は、[@set] コマンドで初期化する際に `meta!` フラグを使用してください。メタ変数は、プレイヤーがルートをクリアした回数や、プレイスルー全体を通じた累計スコアなど、メタ情報や累積情報を示すことができます。

初期値付きの事前定義カスタム変数（メタ変数と通常変数の両方）は、"Custom Variables" 設定メニューで設定できます。

![](https://i.gyazo.com/21701f17403921e34ba4da33b0261ad0.png)

メタの事前定義変数は、アプリケーションの初回起動時に初期化されます。一方、通常の変数は、状態がリセットされるたびに初期化されます。なお、メニューの値フィールドには生の値文字列ではなく、有効なスクリプト式を指定する必要があります。

::: tip
一度だけ増加するメタカウンターが必要な場合（ロールバック時やゲーム再起動後の再プ��イでも再度増加しないようにする場合）は、`hasPlayed()` [式クエリ](/ja/guide/script-expressions#式関数) を使用してください。
...
`meta!` または `const!` フラグを使用すると、デフォルト代入が自動的に適用されるため、`?=` 演算子を明示的に指定する必要はありません。

```nani
; ルートクリア状態を追跡する両方の変数に 'false' を宣言して代入します。
; 同じスクリプトが再度再生された場合（たとえば、次回のゲーム開始時）でも、
; 変数は再代入されません。
