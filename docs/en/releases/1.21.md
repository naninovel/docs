...
- Added `Recovery Rollback` feature to the sate configuration. When enabled (default) will recover the game state when loading a save after modifying played scenario script by rolling back to the start of the script.
...
- `TitleUI` no longer shows automatically on engine initialization and doesn't execute `Title` scenario script on show; instead, the `Title` script is executed automatically on engine initialization and on [@title] command (in case assigned in the scripts configuration). The show the title UI, add `@showUI TitleUI` to the title script.
