...
5. In case using expressions for generic line author (eg, `{mc}: Hi`), use display name instead. ([guide](/guide/characters#display-names))
...
- Added `Render Camera` option to layered actors allowing to render the prefab content with a camera, which lifts all the previous limitations (such as lacking stencil support) and allows rendering non-trivial objects, like particle systems, trails, etc.
...
- Added check for duplicates when drag-dropping script assets to the resources list; the duplicates won't be added and warning will be logged.
...
- `Text ID` auto voice mode (previously `Content Hash`) now supports voice languages.
...
- Development console now supports string parameters with whitespace (via wrapping them in double or single quotes).
...
- Expressions in generic text line author segment are no longer supported; use [display names](/guide/characters#display-names) instead.
...
- `2024-03-01` Fixed script graph losing state when saving and re-entering play mode twice.
