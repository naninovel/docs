...
16. In case using `ExpressionFunctions` attribute to expose custom expression functions, remove the attribute from the class and instead apply `ExpressionFunction` attribute to the methods of the class. [<sup>[doc]</sup>](/guide/expressions#adding-custom-functions)
...
- Removed `ExpressionFunctions` class attribute; use `ExpressionFunction` method attribute instead. [<sup>[doc]</sup>](/guide/expressions#adding-custom-functions)
