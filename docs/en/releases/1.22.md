...
6. In case using `[EnumContext]` metadata attributes, replace them with `[EnumContext]`.
...
- Renamed `[EnumContext]` metadata attribute to `[EnumContext]` to prevent confusion with the newly introcuced expression constants.
