...
You can access the settings JSON file via `File -> Preferences -> Settings` and clicking the "Open Settings (JSON)" button in the upper-right corner of the window. Select the "User" tab to edit settings for all projects or "Workspace" to affect only the current project containing the naninovel scripts.
