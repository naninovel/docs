...
This feature allows presenting a number of choices to the player and rerouting script execution depending on which choice is picked.

![Choices](https://i.gyazo.com/023502e43b35caa706c88fd9ab32003d.png)

Use [@choice] commands followed by the choice summary and an optional `goto` path to add choices from the scenario scripts:
...
The [@choice] command accepts an optional `button` parameter specifying a path (relative to a "Resources" folder) to a custom prefab representing the choice option object.
...
To use a different text component for the choice text, use the `On Summary Text Changed` [Unity event](https://docs.unity3d.com/Manual/UnityEvents) of the choice button component.
...
::: tip EXAMPLE
Find a more advanced implementation of an interactive map with Naninovel in the [map sample](/guide/samples#map).
...
@choice "Answer" handler:ChatReply
    Yuko: school. prepping for the festival
```
